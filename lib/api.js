...
class OpenWaApi {
  constructor(baseUrl) {
    ...
    this.client = axios.create({ baseURL: url, timeout: 20000 });
  }

  sendText(to, content) {
    return this.client.post('/sendText', {
      args: { to: toChatId(to), content }
    });
  }

  sendImage(to, file, filename, caption, options = {}) {
    return this.client.post('/sendImage', {
      args: {
        to: toChatId(to),
        file,
        filename,
